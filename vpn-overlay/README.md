...
`spec.debug_access.ssh_ingress_cidrs` is a separate temporary operator-debug
allowlist for TCP 22 on the two disposable lab nodes. It is also limited to
canonical IPv4 `/32` values and must be removed after debugging. It does not
open WireGuard UDP 51820 or change the desktop VLESS/TLS allowlist. The
separate debug list adds only TCP 22 to both disposable nodes. The desktop
VLESS/TLS list remains disabled unless an explicitly requested desktop window
is enabled; public WireGuard UDP remains closed.
