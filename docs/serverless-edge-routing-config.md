...
- Production `frontend_router.website` separately declares `xworktech.com` and
  `www.xworktech.com` as homepage-only domains in zone `xworktech.com`. They are
  not Console aliases or authentication origins. The same router serves `/`
  and its assets, redirects other read paths to `https://svc.plus`, and rejects
  non-read requests. Deploy the supporting router release before reconciling
  these domains. This declaration does not remove existing legacy bindings.
